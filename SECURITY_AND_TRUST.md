...
[PRD stage dependencies](PRD.md#stage-dependencies) and [ADR0009](docs/adr/0009-store-first-stage-applicability.md) supersede full-program sequencing only for the internal showcase, controlled real-store pilot and paid Gallery pilot. Existing authorization, privacy, content/lifecycle, audit, recovery, funding and provider safeguards remain applicable to the selected exposure and all retained data.

| Exposure | Required evidence and boundary |
| --- | --- |
| Local synthetic showcase | Frozen candidate/fixtures, scoped functional/browser checks, account/store isolation for claimed real-service actions, honest fixture labels and actual owner feedback. No new hosted permission. |
| Hosted synthetic review | Current permitted hosting/protection and an exact valid assessment record. ADR0007/0008 cannot be reused outside their original scope or after expiry. |
| Controlled real-store pilot | Owner-approved stores/participants/data, invitation-assisted admission, actual Auth/email and privileged MFA/scope controls, rights/consent and M-01 before real media, applicable H-01 recovery/protection, L-01 before shared privileged mutation, S-01 legal/trust and HC-01 support/incident capacity, and selected-path human accessibility/security proof. |
| Paid Gallery pilot | All applicable real-store evidence plus exact approved offer/consent, authorized test-mode Checkout/portal/webhook/failed-payment/refund proof, entitlement isolation, incumbent-servicing inventory, current provider/config/CI evidence and explicit live approval under Package 13. |
| Public indexing/discovery, self-service acquisition or promotion | Separate explicit exposure approval, public rate-limit/direct-RPC-denial, B-01/SEC-01/HC-02 and relevant legal/accessibility/recovery evidence. Completing a controlled pilot does not enable these. |

The signed pilot record in [Package 13 and pilot activation](PACKAGE_CONTRACTS.md#store-first-pilot-activation-contract) binds the exact candidate/schema/config, allowed participants/stores/data, routes/commands/jobs, evidence and stop/expiry. The server verifies scope, signature, freshness, expected version and absence of stop; missing, stale, wrong-scope or expired evidence denies. Public registration, real catalog exposure, claim/add intake, public reviews, promotion, routing, trip collaboration and billing each require an explicit capability decision. A record may enable only its allowlist. Negative tests prove direct RPC/Edge/Storage/signed-URL denial for disabled paths; hidden navigation is insufficient. Rollback disables new activity first and preserves necessary servicing/recovery for existing obligations.

This is a plan contract, not an existing implementation or a passed gate. Current server predicates remain closed until separately reviewed code supports the new scope and genuine activation evidence is supplied. No dummy Regional/RG-01/community receipt is permitted. Existing pre-release and public stage procedures remain valid within their original boundaries. Later-feature tests/cohorts, fixed three-store expansion, trip journeys, RG-01 and community counts are not imported as prerequisites to the controlled store/Gallery pilot.
...
- Package 6 tests the workflow with Synthetic data while `public_listing_claims_enabled=false`; public enablement needs the separately approved public-acquisition exposure, not the invited store-pilot receipt. Disabled routes are not found, reads return no records, and direct writes return stage-disabled without disclosing listing/claim existence.
...
Initial pilot onboarding is invited and does not enable public claim/add intake. New sales permit only Gallery; Full Gallery and new paid-to-paid changes below are deferred compatibility contracts. Existing lawful servicing remains available for verified obligations. Stage authorization follows the store-first applicability table, not a compulsory regional/community sequence.
...
- Search-before-add, normalized duplicate detection, rate limits, generic account/store-conflict responses, verified email, MFA, two independent authority signals, exact-store scoping, one-store/one-Representative cardinality, Administrator review, revocation, and audit apply before grant creation. Likely duplicates enter review and convert only through applicant-confirmed existing-store claim; they never create a second listing. Multi-location/bulk claims are denied with a support path. Inactive drafts and rejected/withdrawn personal/unapproved fields purge on the Package 6 schedule. No public waitlist or unapproved public-intake PII store is authorized. The separately signed invited pilot permits only its named participant data.
- Administrator authority/listing approval atomically creates exact scope and default Free; approved new-store applications also create provenance/public projection or roll back all effects. Initial Free-to-paid upgrades require `sales_open`, exact approved Free store scope, MFA/recent authentication, an active immutable commercial-config version, and a fresh immutable consent receipt binding tier, exact monthly price, tax treatment, first charge, automatic renewal, refund/cancellation, proration/downgrade, failed-payment grace, hidden-photo/deletion consequence, applicable offered-target limits, terms/privacy versions, and timestamp. Full Gallery limits are required only for an authorized Full Gallery offer or verified incumbent obligation; a Gallery-only configuration requires no dummy Full Gallery price or limit fields. A stale price/config/version or changed material term invalidates consent and denies Checkout.
...
Apply the matrix to the selected exposed capabilities and retained data under Store-first stage applicability. Deferred trip/routing/review cases remain required for their later activation and do not block a store-only pilot when server-disabled.
...
The following full-program procedure remains the retained historical Alpha/Regional profile. The new bounded store pilot uses Store-first stage applicability and its signed capability inventory; actual auth/scope, audit, privacy, rights, support and recovery duties are retained, while unrelated trip/cohort/program completion is not a prerequisite. No old readiness receipt authorizes new participants, data or hosting.
