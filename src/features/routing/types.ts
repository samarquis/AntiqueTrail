...
export interface SuggestedStop extends CheckMyDayStop {
  arrivalMinute: number
  departureMinute: number
  warning?: string
}
export interface CheckMyDayInput {
  stops: CheckMyDayStop[]
  departureMinute: number
  returnId?: string
  maxDriveMiles?: number
  maxTotalMinutes?: number
  matrix: Record<string, TravelLeg>
  matrixTimestamp: string
}
export interface CheckMyDayResult {
  order: SuggestedStop[]
  legs: TravelLeg[]
  totalMiles: number
  totalMinutes: number
