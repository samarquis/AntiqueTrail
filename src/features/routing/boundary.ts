...
export const ROUTING_BLOCKED_MESSAGE =
  'Map and travel-time suggestions are not available yet. Your list is still available.'
...
export function buildMapRequest(
...
export function buildGeocodeRequest(
  capability: RoutingCapability,
  text: string,
  purpose: GeocodeRequest['purpose'],
): GeocodeRequest | null {
...
function coordinates(point: PublicPoint) {
  return { latitude: point.latitude, longitude: point.longitude }
}

export function mapFallback(capability: RoutingCapability): {
  mapVisible: boolean
  message?: string
} {
  return capability === 'available'
    ? { mapVisible: true }
    : { mapVisible: false, message: ROUTING_BLOCKED_MESSAGE }
}
...
interface Scored {
  result: CheckMyDayResult
  score: [number, number, number, number, number, number, number, string]
}
function evaluate(order: CheckMyDayStop[], input: CheckMyDayInput): Scored {
  let clock = input.departureMinute
  let totalMiles = 0
  let totalTravel = 0
  let late = 0
  const result: SuggestedStop[] = []
  const legs: TravelLeg[] = []
  let previous = '__start__'
  for (const stop of order) {
    const leg = input.matrix[`${previous}->${stop.id}`] ?? {
      from: previous,
      to: stop.id,
      miles: 0,
      minutes: 0,
    ...
    if (stop.kind === 'store' && stop.closesAt != null && clock > stop.closesAt) {
      late += clock - stop.closesAt
      warning = warning ?? 'Arrives after closing'
    }
    const arrivalMinute = clock
    clock += stop.dwellMinutes
    const departureMinute = clock
    result.push({ ...stop, arrivalMinute, departureMinute, warning })
    previous = stop.id
    if (result.length < order.length) clock += 10
  }
  if (input.returnId) {
    const leg = input.matrix[`${previous}->${input.returnId}`] ?? {
      from: previous,
      to: input.returnId,
      miles: 0,
      minutes: 0,
  ...
  const maxExcess =
    Math.max(0, input.maxDriveMiles == null ? 0 : totalMiles - input.maxDriveMiles) +
    Math.max(
      0,
      input.maxTotalMinutes == null ? 0 : clock - input.departureMinute - input.maxTotalMinutes,
    )
  const mustComplete = result.filter((stop) => stop.priority === 'must' && !stop.warning).length
  const points = result
    .filter((stop) => !stop.warning)
    .reduce(
      (sum, stop) => sum + (stop.priority === 'must' ? 3 : stop.priority === 'prefer' ? 2 : 1),
      0,
    )
  const allComplete = result.filter((stop) => !stop.warning).length
  const originalPairs = result
    .slice(1)
    .filter((stop, index) => stop.originalIndex === result[index].originalIndex + 1).length
  const feasible = maxExcess === 0 && late === 0
  const reasons = [
    feasible ? 'Within selected limits.' : 'Shows warnings for limits or store hours.',
    'Use Suggested Order applies only after explicit confirmation.',
    'Keep My Order remains available.',
  ]
  return {
    result: {
      order: result,
      legs,
      totalMiles,
      totalMinutes: totalTravel,
      estimatedFinishMinute: clock,
      feasible,
      reasons,
      matrixTimestamp: input.matrixTimestamp,
    },
    score: [
      feasible ? 0 : 1,
      -mustComplete,
...
function compare(a: Scored, b: Scored): number {
  for (let i = 0; i < a.score.length; i += 1) {
    ...
    if (typeof a.score[i] === 'string') return String(a.score[i]).localeCompare(String(b.score[i]))
