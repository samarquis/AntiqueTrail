...
export function normalizeQueryParams(input: URLSearchParams | string): CatalogFilters {
  ...
  const q = trimUnicode(params.get('q') ?? '').replace(/[\u0000-\u001f\u007f]/gu, '').slice(0, MAX_QUERY_LENGTH);
...
export function formatHours(day: CatalogHoursDay): string {
  ...
  return day.intervals.map((interval) => `${formatTime(interval.opensAt)}–${formatTime(interval.closesAt)}`).join(', ');
}
