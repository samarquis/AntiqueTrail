...
export const GENERIC_SHARE_MESSAGE =
  'If the recipient can receive this share, it will appear in their inbox.'
...
export async function recipientEmailHmac(
  email: string,
  hmac: (normalizedEmail: string) => Promise<string>,
): Promise<string> {
...
export function isCandidateOwner(
...
export function senderShareView(
  userId: string | null | undefined,
  share: CandidateShare,
  now = Date.now(),
): GenericShareEnvelope | null {
  if (!userId || share.senderId !== userId) return null
  if (share.state === 'accepted') return { accepted: true, state: 'accepted', message: 'Accepted' }
  if (share.state === 'pending' && share.expiresAt > now)
    return { accepted: false, state: 'pending', message: 'Pending' }
  return { accepted: false, state: 'closed', message: 'Closed' }
}

export function recipientShareView(
  userId: string | null | undefined,
  share: CandidateShare,
  now = Date.now(),
): GenericShareEnvelope {
  if (!canReadShare(userId, share, now) || share.recipientId !== userId)
    return { accepted: false, state: 'closed', message: GENERIC_CANDIDATE_FAILURE }
  return share.state === 'accepted'
    ? { accepted: true, state: 'accepted', message: 'Accepted' }
    : { accepted: false, state: 'pending', message: 'Pending' }
}
