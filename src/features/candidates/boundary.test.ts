...
describe('candidate private boundary', () => {
  it('normalizes URL capture and rejects SSRF-sensitive destinations', () => {
    expect(normalizeCandidateUrl(' https://Example.com/store#private ')).toEqual({
  ...
  it('isolates sender/recipient and closes expired shares without reason leakage', () => {
    ...
    expect(senderShareView('sender', share, 1_000)).toEqual({
      accepted: false,
      state: 'pending',
      message: 'Pending',
    })
    expect(senderShareView('sender', { ...share, state: 'closed' }, 1_000)).toEqual({
      accepted: false,
      state: 'closed',
      message: 'Closed',
    })
  ...
  it('replays idempotent commands and rejects changed payloads', () => {
    ...
    expect(() => ledger.execute('send-1', { share: 'share-2' }, operation)).toThrow(
