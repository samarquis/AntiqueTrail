...
Authority by domain; later review aids never override the relevant controlling source:
...
6. [`mood.md`](mood.md), [`voice.md`](voice.md), and [`tokens.md`](tokens.md) are **Proposed** review indexes subordinate to the domain-specific authorities above. They enable Mood, Voice, and Token Compliance critiques but create no product policy, copy intent, visual value, palette authority, or icon-placement authority.
7. The HTML flow lab is archival concept provenance only. It does not participate in implementation or acceptance.

Review proposed or changed brand references with [`BRAND_REFERENCE_REVIEW_CHECKLIST.md`](BRAND_REFERENCE_REVIEW_CHECKLIST.md). If a review index conflicts with a controlling source, stop the dependent review or implementation and reconcile the controlling documents; do not silently prefer the index.
