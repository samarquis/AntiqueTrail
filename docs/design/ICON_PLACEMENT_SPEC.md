...
| `saved-store.svg` | A private saved place | `Saved stores` showcase navigation item (or retained trip-stage More item); Saved Stores page header; post-save confirmation next to `Saved` | The heart/favorite action alone; retain the word `Save` or `Saved` |
