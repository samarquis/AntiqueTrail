...
At `ee49fce7`, the browser MFA verify request returned HTTP 200. The subsequent application session registration failed with `admission_required`. `app_public.register_current_session` requires an active global `shopper` role grant, while the `role_grants_store_scope` constraint rejects a store-scoped `shopper` grant. A global shopper grant would make the Administrator eligible for the shopper-private RPC, contradicting the controlling no-shopper-private-access requirement. The attempted store-scoped fixture grant was rejected by the database constraint and was reverted at `351e3702`'s follow-up working change. This is a product/auth admission-boundary defect requiring separately authorized design/implementation work; #323 remains open.
