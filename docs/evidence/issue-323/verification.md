...
The hosted database check is currently blocked by runner infrastructure: the pgTAP job could not pull the pinned `public.ecr.aws/supabase/pg_prove:3.36` image because the registry returned `toomanyrequests` (exit 125), before any database assertions ran. Hosted web, seed-media, sign-out, and plan-governance checks were green on the PR's last published head, but that head predates the current unpushed diagnostic commits.

## Auth-only root-cause diagnosis

At `ee49fce7`, the browser MFA verify request returned HTTP 200. The subsequent application session registration failed with `admission_required`. `app_public.register_current_session` requires an active global `shopper` role grant, while the `role_grants_store_scope` constraint rejects a store-scoped `shopper` grant. A global shopper grant would make the Administrator eligible for the shopper-private RPC, contradicting the controlling requirement that the Administrator has no shopper-private access. The attempted store-scoped fixture grant was rejected by the database constraint and was removed from the published candidate. This is a product/auth admission-boundary defect requiring separately authorized design/implementation work; #323 remains open.
