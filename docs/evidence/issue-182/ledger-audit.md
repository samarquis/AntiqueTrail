...
4. **`OPENCODE_TICKET_REVIEW_TODO.md` — no content correction asserted.** The file is absent in this checkout and not named by `PLANNING_INDEX.md`; confirm whether it is intentionally retired before restoring or creating a ledger. Its absence is a limitation, not evidence of a dependency, tier, or completion defect.
