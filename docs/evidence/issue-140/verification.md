...
- Focused review client/component tests include explicit Dismiss Report case-state wording and outcome assertions.
- `npm run verify:baseline`: 88 files/613 tests, 69 release tests, TypeScript, ESLint, Prettier, production/PWA build.
- Clean local Supabase reset applied migrations `20260903010000` and `20260903020000`; focused pgTAP 0079 passed 22/22, including runtime ownership, execute privilege, stale-version, idempotent replay, positive Hold and eligible Restore, aggregate rebuild, held-dismiss visibility preservation, and ineligible-restore denial.
- Required UI-09 Playwright matrix: 24 passed; two opt-in capture cases skipped.
