...
Fresh review of `23fbc434a9633f6904ae6bfa390e81fb4801ba5d` found a stale current-authority entry in docs/design/README.md and inherited onboarding copy promising email MFA despite Package 2's authenticator-app requirement. The design index now links to README ownership and classifies PRODUCT_DECISIONS.md as history; the guided onboarding copy now describes the existing authenticator requirement. These are documentation consistency corrections, with no application or authentication-method change. Final review and hosted-check results belong to the publication PR's exact candidate record.


The remaining publication findings were preserved and corrected as follows: the Topeka Private Beta boundary now lives in PRD First Pilot Store Record, and the complete six-store controlled comparison protocol moved from USER_RESEARCH into PRD SLM-01 with a research backlink. Acceptance/onboarding/membership/package references now use current owners. The orphan-account language across onboarding, ADR0002, security, and Package 6 distinguishes successfully admitted accounts awaiting partner binding from provider-only orphans with no application access. The inherited social-admission metadata/purge conflict is explicitly unresolved in Security Authentication and blocks only dependent work under existing governance; no replacement auth design or retention exception is invented. Dated storefront exploration is classified as history. These corrections preserve existing scope and required evidence; the PR records the final seven-lens disposition and hosted checks.
