...
**Date**: 2026-08-23 · **Status**: Tickets cut; coding NOT started — awaits explicit Product Owner start instruction (IMPLEMENTATION_PLAN.md line 5)
**Inputs**: DEEP_SPEC_REVIEW.md (2026-08-16 browser review), REVIEW_VERDICTS.md, docs/specs/store-membership-spec.md (#87), PRODUCT_DECISIONS.md §Photo moderation criteria (#92) + §Photo-tier memberships, docs/research/gallery-photo-data-access.md (#95), docs/operations/G56_RELEASE_GATE_STATUS_LEDGER.md (#56)

## Situation summary

Schema groundwork exists for Packages 2B–12 (89 migrations, 31 edge functions), but browser verification covers only Package 4 fully. Eight spec deviations are recorded from the 2026-08-16 review. One entire decided surface — photo tiers, moderation UI, Stripe billing — has no package contract and no implementation. The local Supabase stack cannot boot (#97–#101, #104), which blocks every database-behavior test cycle.

## Workstreams and execution order

### W0 — Local stack unbreak (critical path for all DB TDD)
...
Entry: none. Exit: `npx supabase@2.33.9 start` + `db reset` green; migrations apply clean; public-catalog served locally.

### W1 — Spec-drift burn-down (frontend/test work, no DB dependency)
...
### W2 — Photo-tiers package (staged-off build, owner-approved 2026-08-23)
...
|---|---|---|
| #113 | [task] Define Package 13 acceptance contract in PACKAGE_CONTRACTS.md — OWNER REVIEWS BEFORE MIGRATION | — |
...
### W3 — Contract verification sweep (interleave after W1)
| Ticket | Slice |
|---|---|
...
### W4 — Provider/human gate prep (NOT ticketed yet — trigger-based)
Cut tickets only when their trigger fires: E-01/M-01/R-01/L-01/H-01 drill scripts when provider selection starts; S-01/HC-02 rehearsal packs when human names exist. Tracked by #56 ledger rows. Do not cut early — ticket rot.
