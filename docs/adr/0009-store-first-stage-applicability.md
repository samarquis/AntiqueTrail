...
- Status: Accepted by the Product Owner's 2026-09-11 directive; effective on reviewed merge of #346, with the bounded local trip-work exception authorized on 2026-09-12 by PR #360 and extended to #365 only when its dedicated reviewed amendment merges. No implementation or activation receipt.
...
The bounded trip exception uses only fictional, run-owned local identities and fixtures through actual local Auth/Edge/RPC transport. It must preserve verified-recipient binding, one-trip isolation, creator-only accepted-partner removal, author-private data, offline/device authority revocation and next-request denial. #365 may grant only the existing verified-email HMAC verifier's owner the private helper execution needed for invitation acceptance; `public`, `anon`, `authenticated` and unrelated service roles retain no direct helper execution. It grants no shared hosting, external account or participant, real data, email delivery, provider operation, spending, deployment, showcase/pilot inclusion or production capability. Work outside #321/#342/#343/#344/#365 remains deferred.
