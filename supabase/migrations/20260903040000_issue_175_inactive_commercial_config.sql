...
create or replace function partner_private.commercial_config_is_complete(
  ...
  select p_config.gallery_price_cents > 0
    and p_config.full_gallery_price_cents > 0
    and p_config.full_gallery_price_cents >= p_config.gallery_price_cents
    and p_config.currency ~ '^[A-Z]{3}$'
    and btrim(coalesce(p_config.tax_mode,'')) <> ''
    and btrim(coalesce(p_config.first_charge_rule,'')) <> ''
    and btrim(coalesce(p_config.renewal_rule,'')) <> ''
    and btrim(coalesce(p_config.refund_policy_version,'')) <> ''
    and btrim(coalesce(p_config.support_policy_version,'')) <> ''
    and btrim(coalesce(p_config.terms_version,'')) <> ''
    and btrim(coalesce(p_config.privacy_version,'')) <> ''
    and btrim(coalesce(p_config.full_gallery_limits_version,'')) <> ''
    ...
    and jsonb_array_length(p_config.full_gallery_limits->'acceptedFileTypes') > 0
    and jsonb_typeof(p_config.full_gallery_limits->'maxFileBytes') = 'number'
    and jsonb_typeof(p_config.full_gallery_limits->'maxWidthPixels') = 'number'
    and jsonb_typeof(p_config.full_gallery_limits->'maxHeightPixels') = 'number'
    and (p_config.full_gallery_limits->>'maxFileBytes')::bigint > 0
    and (p_config.full_gallery_limits->>'maxWidthPixels')::integer > 0
    and (p_config.full_gallery_limits->>'maxHeightPixels')::integer > 0
    and not (p_config.full_gallery_limits ? 'normalOperationCountCap')
    and btrim(p_config.full_gallery_limits->>'uploadRateRule') <> ''
    and btrim(p_config.full_gallery_limits->>'quotaOutageRule') <> ''
    and btrim(p_config.full_gallery_limits->>'moderationAbuseRule') <> ''
    and btrim(p_config.full_gallery_limits->>'reasonRecoveryAppealRule') <> ''
    and btrim(p_config.full_gallery_limits->>'paidServiceRemedy') <> ''
$$;
...
begin
  ...
  if config.version is null or v_authorization.authorization_id is null
    or v_authorization.config_version <> p_version or v_authorization.state <> 'active'
  ...
  update partner_private.photo_tier_commercial_configs set
    ...
    research_authorization_id = v_authorization.authorization_id,
    approved_by = v_authorization.signed_by, approved_at = v_authorization.signed_at
    where version = p_version;
  return jsonb_build_object('version',p_version,'state','approved_inactive','digest',encode(frozen_digest,'hex'));
end $$;
