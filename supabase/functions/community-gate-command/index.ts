...
import { parseCommunityGateCommand } from '../_shared/community-user-command.ts'
...
Deno.serve(async (request) => {
  ...
  try {
    const command = parseCommunityGateCommand(await request.json())
    const client = createClient(url, anonKey, {
      db: { schema: 'app_public' },
      global: { headers: { authorization } },
      auth: { persistSession: false, autoRefreshToken: false },
    })
    const result = await client.rpc('community_gate_command', {
