...
select ok(position('trip_private.trips' in lower(pg_get_functiondef('rg01_private.derive_source_fact_before_live_scope(text,uuid)'::regprocedure)))>0,'trip facts derive from server trips and stops');
select ok(position('app_public.stores' in lower(pg_get_functiondef('rg01_private.derive_source_fact_before_live_scope(text,uuid)'::regprocedure)))>0 and position('catalog_freshness' in lower(pg_get_functiondef('rg01_private.derive_source_fact_before_live_scope(text,uuid)'::regprocedure)))>0,'listing facts derive from the live public catalog and freshness');
select ok(position('readiness_private.readiness_fact_events' in lower(pg_get_functiondef('rg01_private.derive_source_fact_before_live_scope(text,uuid)'::regprocedure)))>0,'defect facts bind the authoritative defect record');
...
select ok(position('partner_private.listing_claims' in lower(pg_get_functiondef('rg01_private.derive_source_fact_before_live_scope(text,uuid)'::regprocedure)))>0,'claim facts bind the authoritative claim record');
...
select ok(position('sync_authoritative_source_facts' in lower(pg_get_functiondef('rg01_private.freeze_run(uuid)'::regprocedure)))>0 and position('authoritative_source_coverage_complete' in lower(pg_get_functiondef('rg01_private.freeze_run(uuid)'::regprocedure)))>0,'freeze synchronizes and rechecks the authoritative source set');
select ok(position('authoritative_source_ids' in lower(pg_get_functiondef('rg01_private.source_head_digest()'::regprocedure)))>0,'receipt currency changes when a domain source changes');
...
select ok(position('bound_release_id' in lower(pg_get_functiondef('app_public.rg01_set_flyer_consent(uuid,boolean,bytea)'::regprocedure)))>0 and position('promotion_consent_receipt_digest' in lower(pg_get_functiondef('app_public.rg01_set_flyer_consent(uuid,boolean,bytea)'::regprocedure)))>0,'flyer consent checks the server capability');
