...
begin;
insert into auth.users(id) values ('25600000-0000-4000-8000-000000000901');
insert into auth.sessions(id, user_id, created_at, updated_at) values ('25600000-0000-4000-8000-000000000911', '25600000-0000-4000-8000-000000000901', statement_timestamp(), statement_timestamp());
insert into app_private.profiles(user_id, verified_email_snapshot, age_18_attested_at) values ('25600000-0000-4000-8000-000000000901', 'issue256-c@example.invalid', statement_timestamp());
insert into app_private.role_grants(subject_user_id, role) values ('25600000-0000-4000-8000-000000000901', 'shopper');
insert into trip_private.trips(trip_id, owner_id, area_id, name, local_date, version)
select '25600000-0000-4000-8000-000000000991', '25600000-0000-4000-8000-000000000901', id, 'Conflict trip', date '2026-09-08', 20
...
values ('25600000-0000-4000-8000-000000000992', '25600000-0000-4000-8000-000000000991', 'store', (select id from app_public.stores order by id limit 1), 0),
       ('25600000-0000-4000-8000-000000000993', '25600000-0000-4000-8000-000000000991', 'store', (select id from app_public.stores order by id offset 1 limit 1), 1);
commit;

select set_config('request.jwt.claims', '{"sub":"25600000-0000-4000-8000-000000000901","role":"authenticated","session_id":"25600000-0000-4000-8000-000000000911"}', false);
...
select set_config('test.first_competing_edit', app_public.set_trip_stop_priority('25600000-0000-4000-8000-000000000991', '25600000-0000-4000-8000-000000000992', 'must', 20)::text, false);
select is((current_setting('test.first_competing_edit')::jsonb->>'version')::bigint, 21::bigint, 'first same-version edit succeeds');
select throws_ok($$select app_public.set_trip_stop_dwell('25600000-0000-4000-8000-000000000991','25600000-0000-4000-8000-000000000993',30,20)$$, 'P0001', 'conflict', 'second same-version edit returns a conflict');
reset role;
select is((select s.priority from trip_private.trip_stops as s where s.stop_id = '25600000-0000-4000-8000-000000000992'::uuid), 'must', 'first competing edit is persisted');
select is((select s.planned_dwell_minutes::integer from trip_private.trip_stops as s where s.stop_id = '25600000-0000-4000-8000-000000000993'::uuid), 60, 'conflicting competing edit does not mutate its stop');
select is((select t.version from trip_private.trips as t where t.trip_id = '25600000-0000-4000-8000-000000000991'::uuid), 21::bigint, 'conflicting competing edit does not increment the trip version');
