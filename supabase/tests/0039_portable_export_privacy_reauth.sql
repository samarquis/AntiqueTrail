...
select ok(position($q$array['password']$q$ in regexp_replace(pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure),'[[:space:]]','','g'))>0,'fresh password AMR is mandatory');
