...
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='pending_identity_bound_read' and replace(coalesce(qual,''),' ','') like '%auth_user_id=auth.uid()%'),'pending identity read is bound-user scoped');
...
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='listing_claim_claimant_read' and replace(coalesce(qual,''),' ','') like '%claimant_id=auth.uid()%'),'claim read is claimant scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='claim_signal_claimant_read' and coalesce(qual,'') like '%listing_claims%' and replace(coalesce(qual,''),' ','') like '%claimant_id=auth.uid()%'),'claim signals are sibling-row scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='partner_grant_exact_owner_read' and replace(coalesce(qual,''),' ','') like '%auth_user_id=auth.uid()%' and coalesce(qual,'') like '%current_session_is_active%'),'grant read is exact-user scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='consent_receipt_bound_read' and replace(coalesce(qual,''),' ','') like '%auth_user_id=auth.uid()%'),'consent receipt read is bound-user scoped');
