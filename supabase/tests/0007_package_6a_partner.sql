...
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='pilot_draft_bound_owner' and coalesce(qual,'') like '%pending_partner_identities%' and coalesce(qual,'') like '%auth.uid()%'),'draft read/write is pending-identity scoped');
