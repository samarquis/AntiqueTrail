...
select ok((select count(*)=13 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='partner_private' and c.relkind='r' and c.relrowsecurity and c.relforcerowsecurity),'all Package 6A tables FORCE RLS');
...
select ok(exists(select 1 from pg_index i join pg_class c on c.oid=i.indexrelid where c.relname='store_partnership_active_store_idx' and i.indisunique),'one active partnership per store');
select ok(exists(select 1 from pg_index i join pg_class c on c.oid=i.indexrelid where c.relname='partner_grant_active_store_idx' and i.indisunique),'one active grant per store');
select ok(exists(select 1 from pg_index i join pg_class c on c.oid=i.indexrelid where c.relname='partner_grant_active_user_store_idx' and i.indisunique),'one active grant per user/store');
...
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='pending_identity_bound_read' and coalesce(qual,'') like '%auth_user_id=auth.uid()%'),'pending identity read is bound-user scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='pilot_draft_bound_owner' and (coalesce(qual,'') like '%pending_partner_identities%' or coalesce(qual,'') like '%pilot_draft_belongs_to_user%') and coalesce(qual,'') like '%auth.uid%'),'draft read/write is pending-identity scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='listing_claim_claimant_read' and coalesce(qual,'') like '%claimant_id=auth.uid()%'),'claim read is claimant scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='claim_signal_claimant_read' and coalesce(qual,'') like '%listing_claims%' and coalesce(qual,'') like '%claimant_id=auth.uid()%'),'claim signals are sibling-row scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='partner_grant_exact_owner_read' and coalesce(qual,'') like '%auth_user_id=auth.uid()%' and coalesce(qual,'') like '%current_session_is_active%'),'grant read is exact-user scoped');
select ok(exists(select 1 from pg_policies where schemaname='partner_private' and policyname='consent_receipt_bound_read' and coalesce(qual,'') like '%auth_user_id=auth.uid()%'),'consent receipt read is bound-user scoped');
