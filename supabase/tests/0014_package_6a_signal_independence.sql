...
select ok(not has_table_privilege('authenticated','partner_private.listing_claims','INSERT') and not exists(select 1 from pg_policies where schemaname='partner_private' and tablename='listing_claims' and policyname='listing_claim_claimant_write'),'browser claim insertion stays disabled until the trusted release service enables it');
