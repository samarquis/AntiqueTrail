...
select ok(position($q$'shopper'$q$ in lower(pg_get_functiondef('app_public.candidate_edge_context()'::regprocedure)))>0,'Shopper is an allowed Candidate role');
select ok(position($q$'representative'$q$ in lower(pg_get_functiondef('app_public.candidate_edge_context()'::regprocedure)))>0,'Representative is an allowed Candidate role');
select ok(position($q$'administrator'$q$ in lower(pg_get_functiondef('app_public.candidate_edge_context()'::regprocedure)))>0,'Administrator is an allowed Candidate role');
...
select ok(position($q$share_row.state<>'pending'$q$ in replace(pg_get_functiondef('app_public.revoke_candidate_share(uuid,text)'::regprocedure),' ',''))>0,'only pending shares may be revoked');
select ok(position($q$action='revoke'$q$ in replace(pg_get_functiondef('app_public.revoke_candidate_share(uuid,text)'::regprocedure),' ',''))>0,'revoke supports idempotent replay');
select ok(position($q$close_reason='revoked'$q$ in replace(pg_get_functiondef('app_public.revoke_candidate_share(uuid,text)'::regprocedure),' ',''))>0,'revoke uses the canonical terminal reason');
