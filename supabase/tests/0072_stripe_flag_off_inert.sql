...
select ok(not has_function_privilege('billing_mirror_service','partner_private.billing_apply_subscription_event(text,text,timestamptz,uuid,text,text,text,timestamptz,text)','EXECUTE') and has_function_privilege('billing_mirror_service','app_public.billing_record_subscription_event(text,text,timestamptz,uuid,text,text,text,timestamptz,text)','EXECUTE'),'mirror service must use the consent-bound subscription transport');
