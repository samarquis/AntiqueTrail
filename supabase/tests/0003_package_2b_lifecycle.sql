...
set local role anon;
select throws_ok($$select * from app_private.account_admission_receipts$$,'42501',null,'anonymous admission reads denied');
...
select throws_ok($$update app_private.account_admission_receipts set state='claimed'$$,'42501',null,'admission rows cannot be directly updated by runtime role');
select throws_ok($$insert into app_private.account_admission_receipts(token_hash,purpose,email_hmac,age_18_attested_at,idempotency_key,claim_expires_at) values (extensions.digest('short','sha256') || decode('00','hex'), 'shopper', extensions.digest('email','sha256'), statement_timestamp(), 'invalid', statement_timestamp())$$,'23514',null,'invalid admission hash is rejected');
select * from finish();
rollback;
