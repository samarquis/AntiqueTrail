...
select ok(not exists(select 1 from information_schema.role_table_grants where table_schema='trip_private' and grantee in ('anon','authenticated') and not (grantee='authenticated' and table_name='trip_duplicate_stop_warnings' and privilege_type='SELECT')),'only the exact owner-scoped duplicate-warning projection has an authenticated table grant');
...
select ok(exists(select 1 from pg_constraint c join pg_class r on r.oid=c.conrelid join pg_namespace n on n.oid=r.relnamespace where n.nspname='trip_private' and r.relname='trip_offline_grants' and c.conname='offline_expiry_bound' and pg_get_constraintdef(c.oid) like '%36:00:00%'),'offline grant expiry is capped at 36 hours');
