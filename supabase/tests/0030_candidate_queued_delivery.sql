...
select ok(position("'queued'" in pg_get_functiondef('app_public.candidate_enqueue_share_delivery(uuid,uuid,bytea,bytea,text)'::regprocedure))>0,'enqueue records durable queued receipt');
...
select ok(position("state='processing'" in replace(pg_get_functiondef('app_public.candidate_claim_share_delivery(uuid)'::regprocedure),' ',''))>0,'claim creates processing lease');
...
select ok(position("result:='delivered'" in replace(pg_get_functiondef('app_public.candidate_complete_share_delivery(uuid,uuid,uuid,bytea)'::regprocedure),' ',''))>0,'matched eligible delivery is recorded');
select ok(position("resulttext:='no_delivery'" in replace(pg_get_functiondef('app_public.candidate_complete_share_delivery(uuid,uuid,uuid,bytea)'::regprocedure),' ',''))>0,'unmatched unverified or blocked delivery is reason-neutral');
...
select ok(position("next_state:=casewhendelivery.attempts>=5then'dead'else'retry'end" in replace(pg_get_functiondef('app_public.candidate_fail_share_delivery(uuid,uuid)'::regprocedure),' ',''))>0,'failure retries up to five attempts');
