...
select ok(position($q$array['password']$q$ in pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure))>0,'privileged proof requires fresh password AMR');
select ok(position($q$array['totp','recovery_code']$q$ in pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure))>0,'privileged proof requires fresh enrolled MFA AMR');
select ok(position($q$claims->>'aal'='aal2'$q$ in pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure))>0,'privileged MFA proof requires provider AAL2');
select ok(position('least(' in pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure))>0
  and position($q$interval '10 minutes'$q$ in pg_get_functiondef('app_private.current_session_has_privileged_reauth(interval)'::regprocedure))>0,'callers cannot widen the ten-minute ceiling');
...
select ok(position($q$interval '10 minutes'$q$ in pg_get_functiondef('app_private.current_session_has_mfa()'::regprocedure))>0,'MFA gate rejects ancient AMR');
