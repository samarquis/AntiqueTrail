...
select ok(position('*0.75' in replace(lower(pg_get_functiondef('app_public.routing_reserve_operation(text,uuid,boolean,integer,jsonb,integer)'::regprocedure)),' ',''))>0 and position($q$set state='paused'$q$ in replace(lower(pg_get_functiondef('app_public.routing_reserve_operation(text,uuid,boolean,integer,jsonb,integer)'::regprocedure)),' ',''))>0,'safe quota latch pauses at seventy-five percent');
