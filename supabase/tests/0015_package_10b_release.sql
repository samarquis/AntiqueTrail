...
select ok(position("mode='closed'" in pg_get_functiondef('release_private.rollback_regional_release(uuid,uuid,text)'::regprocedure))>0 and position("audience='regional_readiness'" in pg_get_functiondef('release_private.rollback_regional_release(uuid,uuid,text)'::regprocedure))>0,'rollback closes registration and withdraws the promoted catalog');
