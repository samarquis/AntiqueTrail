...
select ok(position('current_session_has_mfa()' in lower(pg_get_functiondef('beta_private.require_product_owner()'::regprocedure)))>0
  ...
  and position($q$responsibility='productowner'$q$ in replace(lower(pg_get_functiondef('beta_private.require_product_owner()'::regprocedure)),' ',''))>0,'Product Owner decisions require responsibility, MFA, and recent auth');
...
select ok(position($q$pg_advisory_xact_lock(hashtextextended('beta-gate-evidence'$q$ in replace(lower(pg_get_functiondef('app_public.beta_request_gate_decision(uuid,smallint,text)'::regprocedure)),' ',''))>0
  and position($q$pg_advisory_xact_lock(hashtextextended('beta-gate-evidence'$q$ in replace(lower(pg_get_functiondef('app_public.beta_complete_gate_decision(uuid,text,text)'::regprocedure)),' ',''))>0,'freeze and completion hold the same evidence lock through challenge or receipt insertion');
...
select ok(position('next_ordinal - 1' in lower(pg_get_functiondef('app_public.beta_admit_next_store(uuid,uuid,uuid,bigint,text)'::regprocedure)))>0
  and position($q$decision = 'pass'$q$ in lower(pg_get_functiondef('app_public.beta_admit_next_store(uuid,uuid,uuid,bigint,text)'::regprocedure)))>0,'the prior ordinal needs a passing signed receipt');
...
select ok(position('catalog_freshness' in lower(pg_get_functiondef('app_public.beta_admit_next_store(uuid,uuid,uuid,bigint,text)'::regprocedure)))>0
  and position($q$= 'current'$q$ in lower(pg_get_functiondef('app_public.beta_admit_next_store(uuid,uuid,uuid,bigint,text)'::regprocedure)))>0,'admission requires current authoritative catalog facts');
...
select ok(position($q$audience = 'private_beta'$q$ in lower(pg_get_functiondef('app_public.beta_admit_next_store(uuid,uuid,uuid,bigint,text)'::regprocedure)))>0,'real admitted store receives only private-beta audience');
select ok(position($q$publication_state = 'hidden'$q$ in lower(pg_get_functiondef('app_public.beta_withdraw_store(uuid,uuid,text,bigint,text)'::regprocedure)))>0
  and position($q$state = 'revoked'$q$ in lower(pg_get_functiondef('app_public.beta_withdraw_store(uuid,uuid,text,bigint,text)'::regprocedure)))>0,'withdrawal hides the exact store and revokes exact beta visibility');
...
select ok(position($q$state = 'paused'$q$ in lower(pg_get_functiondef('app_public.beta_refresh_operational_latch(timestamp with time zone)'::regprocedure)))>0
  and position($q$publication_state = 'hidden'$q$ in lower(pg_get_functiondef('app_public.beta_refresh_operational_latch(timestamp with time zone)'::regprocedure)))>0,'latch failure pauses cohorts and hides active beta stores');
...
select ok(position($q$c.state <> 'paused'$q$ in lower(pg_get_functiondef('app_public.beta_recover_cohort(uuid,bigint,text)'::regprocedure)))>0
  and position($q$operational_state = 'current'$q$ in lower(pg_get_functiondef('app_public.beta_recover_cohort(uuid,bigint,text)'::regprocedure)))>0
...
select ok(position($q$ordinal = 3$q$ in lower(pg_get_functiondef('app_public.beta_complete_gate_decision(uuid,text,text)'::regprocedure)))>0
