...
select ok(position($q$array['password']$q$ in pg_get_functiondef('app_private.current_session_recent_auth(interval)'::regprocedure))>0,'recent auth requires password AMR');
...
select ok(position($q$claims->>'aal'='aal2'$q$ in pg_get_functiondef('app_private.current_session_has_mfa()'::regprocedure))>0,'MFA requires provider AAL2');
select ok(position($q$'totp','recovery_code'$q$ in pg_get_functiondef('app_private.current_session_has_mfa()'::regprocedure))>0,'MFA proof accepts enrolled TOTP or recovery fallback');
