...
select ok(position("statement_timestamp() + '00:30:00'::interval" in pg_get_functiondef('app_public.issue_synthetic_partner_invitation(bytea,smallint,text)'::regprocedure))>0,'invitation expiry is exactly thirty minutes');
...
select ok(position("partner_claimant_claim_command('recheck'" in lower(pg_get_functiondef('app_public.partner_synthetic_command(text,jsonb)'::regprocedure)))>0,'Synthetic recheck uses the durable claimant command');

select ok(position("channel_class = 'published_business_contact'" in lower(pg_get_functiondef('partner_private.approve_exact_claim(uuid,uuid)'::regprocedure)))>0,'approval requires the business-domain or published-contact class');
...
select ok(position("pg_advisory_xact_lock(pg_catalog.hashtextextended('partner-store:'" in lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)))>0,'Administrator mutation locks the exact store');
...
select ok(position("state = 'revoked'" in lower(pg_get_functiondef('partner_private.revoke_exact_claim_scope(uuid,uuid,text,text)'::regprocedure)))>0 and position('app_private.role_grants' in lower(pg_get_functiondef('partner_private.revoke_exact_claim_scope(uuid,uuid,text,text)'::regprocedure)))>0,'revoke removes both claim grant and runtime Representative role');

select ok(position("'conflict'" in lower(pg_get_functiondef('app_public.partner_claim_status(uuid)'::regprocedure)))>0 and position("'in_review'" in lower(pg_get_functiondef('app_public.partner_claim_status(uuid)'::regprocedure)))>0 and position("'rejected'" in lower(pg_get_functiondef('app_public.partner_claim_status(uuid)'::regprocedure)))>0 and position("'closed'" in lower(pg_get_functiondef('app_public.partner_claim_status(uuid)'::regprocedure)))>0,'claimant status conflates internal conflict and closure reasons');
...
select ok(position('s.synthetic' in lower(pg_get_functiondef('partner_private.claim_stage_allowed(uuid)'::regprocedure)))>0 and position("'synthetic_alpha'" in lower(pg_get_functiondef('partner_private.claim_stage_allowed(uuid)'::regprocedure)))>0,'Synthetic claims are limited to Synthetic Alpha');
select ok(position('not s.synthetic' in lower(pg_get_functiondef('partner_private.claim_stage_allowed(uuid)'::regprocedure)))>0 and position('public_capability_enabled' in lower(pg_get_functiondef('partner_private.claim_stage_allowed(uuid)'::regprocedure)))>0 and position("'claims'" in lower(pg_get_functiondef('partner_private.claim_stage_allowed(uuid)'::regprocedure)))>0,'real claims stay disabled until the real Package 10B capability');
