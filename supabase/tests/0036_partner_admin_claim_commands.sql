...
select ok(position($q$statement_timestamp()+interval'30minutes'$q$ in regexp_replace(lower(pg_get_functiondef('app_public.issue_synthetic_partner_invitation(bytea,smallint,text)'::regprocedure)),'[[:space:]]','','g'))>0,'invitation expiry is exactly thirty minutes');
...
select ok(position($q$values(p_claim_id,p_channel_class,p_signal_type,'submitted'$q$ in regexp_replace(lower(pg_get_functiondef('partner_private.record_synthetic_claim_signal(uuid,text,text,bytea)'::regprocedure)),'[[:space:]]','','g'))>0,'signal submission can only create submitted state');
select ok(position('c.claimant_id<>actor' in regexp_replace(lower(pg_get_functiondef('partner_private.record_synthetic_claim_signal(uuid,text,text,bytea)'::regprocedure)),'[[:space:]]','','g'))>0,'signal submission denies sibling claim identifiers');
select ok(position($q$s.status<>'submitted'$q$ in regexp_replace(lower(pg_get_functiondef('partner_private.verify_synthetic_claim_signal(uuid,uuid,bytea,uuid,text)'::regprocedure)),'[[:space:]]','','g'))>0,'verification consumes only a submitted signal');
select ok(position('p_verifier_user_id=c.claimant_id' in regexp_replace(lower(pg_get_functiondef('partner_private.verify_synthetic_claim_signal(uuid,uuid,bytea,uuid,text)'::regprocedure)),'[[:space:]]','','g'))>0,'claimant self-verification is denied');
select ok(position($q$role='administrator'$q$ in regexp_replace(lower(pg_get_functiondef('partner_private.verify_synthetic_claim_signal(uuid,uuid,bytea,uuid,text)'::regprocedure)),'[[:space:]]','','g'))>0,'signal verifier must be an active Administrator');
...
select ok(position('evidencereference' in lower(pg_get_functiondef('app_public.partner_synthetic_command(text,jsonb)'::regprocedure)))=0 and position('evidencerefhmac' in lower(pg_get_functiondef('app_public.partner_synthetic_command(text,jsonb)'::regprocedure)))>0,'database command receives only the evidence HMAC');
...
select ok(position($q$channel_class='published_business_contact'$q$ in regexp_replace(lower(pg_get_functiondef('partner_private.approve_exact_claim(uuid,uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'approval requires the business-domain or published-contact class');
...
select ok(position('c.claimant_id=p_actor' in regexp_replace(lower(pg_get_functiondef('partner_private.approve_exact_claim(uuid,uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'Administrator self-approval is denied');
...
select ok(position('frompartner_private.listing_claims' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g'))>0 and position('store_id=c.store_id' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g'))>0 and position('forupdate' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'Administrator mutation locks every competing exact-store claim');
select ok(position($q$p_idempotency_key||'-old');performpartner_private.approve_exact_claim(c.claim_id$q$ in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'transfer revokes old exact scope before approving new scope');
select ok(position($q$state='revoked'$q$ in regexp_replace(lower(pg_get_functiondef('partner_private.revoke_exact_claim_scope(uuid,uuid,text,text)'::regprocedure)),'[[:space:]]','','g'))>0 and position('app_private.role_grants' in lower(pg_get_functiondef('partner_private.revoke_exact_claim_scope(uuid,uuid,text,text)'::regprocedure)))>0,'revoke removes both claim grant and runtime Representative role');
...
select ok(position('assigned_admin_idisnullorassigned_admin_id=actor' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_case(uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'Administrator reads only one assigned or unassigned exact case');
select ok(position($q$claim_id=c.claim_idandstatus='submitted'$q$ in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_case(uuid)'::regprocedure)),'[[:space:]]','','g'))>0,'Administrator signal projection is exact-claim scoped');
