...
select ok(position('revoke_exact_claim_scope(old.claim_id' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g'))<position('approve_exact_claim(c.claim_id' in regexp_replace(lower(pg_get_functiondef('app_public.partner_admin_claim_command(text,uuid,bigint,text,text,uuid)'::regprocedure)),'[[:space:]]','','g')),'transfer revokes old exact scope before approving new scope');
