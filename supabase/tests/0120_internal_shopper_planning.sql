...
select lives_ok($$select app_public.internal_review_pre_request()$$,'configured client store-add endpoint admitted');
select is(jsonb_array_length(app_public.add_trip_store_stop(current_setting('test.owned_trip'),'00000000-0000-4000-8000-000000001001')->'stops'),1,'configured client adds owned catalog stop');
select throws_ok($$select app_public.add_trip_store_stop(current_setting('test.owned_trip'),'99000000-0000-4000-8000-000000001099')$$,'P0001','store_stop_not_found','store ID endpoint denies unowned store');
...
select set_config('request.path','rpc/add_trip_store_stop',true);
select lives_ok($$select app_public.internal_review_pre_request()$$,'sibling has its own planning admission');
select throws_ok($$select app_public.add_trip_store_stop(current_setting('test.owned_trip'),'00000000-0000-4000-8000-000000001002')$$,'P0001','authorization_lost','store ID endpoint denies foreign account trip');
select set_config('request.path','rpc/rename_trip',true);
