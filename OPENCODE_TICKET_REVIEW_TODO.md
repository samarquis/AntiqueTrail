...
- [F] 01. #123 — Rejected-media resubmission against the current server tier resolver. Candidate source-and-evidence SHA `ca17724` (PR #190) repairs the review findings: state-aware same-key retry, no weak preflight, dedicated Official photos history/resubmit UX, distinct state/primary action treatment, and synthetic desktop/mobile, text-spacing, Chromium 200% page-scale, dark, and forced-colors proof. Local verification passed: clean reset plus pgTAP (78 files / 2,160 assertions), focused Vitest commands (31, 41, and 53 tests), UI-08 (16 runnable; 4 intentional capture skips), build, format, typecheck, lint, security contract, plan-governance contract, and `git diff --check`. Blocked from merge/closure: #174 is explicitly a prerequisite and remains OPEN with its own recorded defect; hosted checks for the latest documentation-only head remain pending. Prior failed review: https://github.com/samarquis/AntiqueTrail/issues/123#issuecomment-5483682183. Not merged/closed.
