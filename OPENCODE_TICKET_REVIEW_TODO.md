...
- [x] 01. #123 — Rejected-media resubmission against the current server tier resolver. SECOND REPAIR REVIEW REQUEST 2026-08-31: base `186e7b7`, repair head `d89b245`, PR #190. The exact candidate closes the first repair-review findings: completed same-key retries short-circuit storage; resubmission no longer uses a weak browser-callable preflight; the dedicated Official photos route exposes status/history/rejection/resubmit semantics; and UI-08 adds the synthetic journey at desktop/mobile, 320px text spacing, dark, and forced-colors. A clean local Supabase reset followed by full pgTAP passed (78 files / 2,160 assertions); focused Vitest commands passed (31, 41, and 53 tests); full UI-08 passed (16 runnable; 4 intentional opt-in capture skips); build, format, typecheck, lint, security contract, plan-governance contract, and `git diff --check` passed. Requires a separate-agent review of `d89b245` and fresh hosted checks before merge/closure. Prior failed review: https://github.com/samarquis/AntiqueTrail/issues/123#issuecomment-5483682183. Not merged/closed.
