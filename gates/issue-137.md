...
- [x] G1: One typed route-parent registry classifies every mounted authorized Administrator route and produces exactly one active parent.
  ...
  EXPECT: Test Files
  EVIDENCE: 2026-09-02: App shell and registry coverage pass; `ADMIN_ROUTES` is the single source for mounting and parent classification. Full baseline passed 89 files and 612 tests.
...
- [x] G3: Ticket security and repository floor pass on the candidate.
  ...
  EXPECT: PASS
  EVIDENCE: 2026-09-02 exact `npm run verify:baseline` passed under declared Node 20.19.0: 89 files/612 tests, 69 release tests, and production/PWA build. Lint had zero errors or warnings; security and `git diff --check` passed.
