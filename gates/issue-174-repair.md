...
The R8/R9 rows above became evidence-able only after PR #193 merged, so the completed ledger commit (`6606b85`) was carried to `main` through follow-up PR #195 to keep the acceptance record on the default branch. PR #193's required suite was green on its exact reviewed head (`database`, `web`, `plan-governance` all pass on `79eb650`); PR #195 is a doc-only delta over rows already proven, and re-passed `plan-governance` on head `632d316` (run 33565247822). An earlier carrier PR #194 on the post-merge repair branch did not dispatch the `pull_request` CI workflow (web/database never registered), so the carrier moved to a fresh branch `codex/issue-174-gates-ledger`; PR #194 was closed as unmergeable.