...
test('shopper navigation stays legible and stage-correct at narrow and desktop widths', async ({
  page,
}) => {
  for (const viewport of [
    { width: 1440, height: 1000 },
    ...
    for (const identity of ['anonymous', 'shopper-a']) {
      ...
      await expect(primary.getByRole('link', { name: /trip|create new account/i })).toHaveCount(0)
      ...
      expect(geometry.every(({ left, right }) => left >= 0 && right <= viewport.width)).toBe(true)
