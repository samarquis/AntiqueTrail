...
- **Onboarding flow:** onboarding starts during pilot consent — Administrator-generated Store Partner Invitation → consent receipt — never a public signup page. The owner submits core listing groups (identity/location, contact, hours, categories) plus optional description; photos come after tier selection. Tier selection happens after draft listing creation and before publish. Payment runs through Stripe's hosted Checkout page via redirect (no card fields on Antique Trail). Payment success leads to Administrator approval before the listing publishes; payment alone never grants publication. Owners reach their dashboard through normal PWA sign-in (verified email + MFA), matching the Representative Activation Handoff — no magic links. Approved 2026-08-23.
- **Pilot grandfathering:** existing pilot stores with approved Pilot Store Records receive the free tier automatically at the current media cap (cover plus five approved images). Their consent receipts stay unchanged and unpaid; there is no deadline or grandfathering window that expires. Stores that never upgrade keep the free tier indefinitely and are never removed from the catalog for not paying — removal happens only through existing withdrawal or verification-failure paths. A store that chooses a paid tier signs a new Pilot Consent Receipt including payment terms (material term change requires a new receipt); staying free requires nothing. Approved 2026-08-23.
