...
1. Read `PLAN_GOVERNANCE.md`, `PROJECT_STATE.md`, `PLANNING_INDEX.md`, `PRD.md`, `DESIGN.md`, `DESIGN_SYSTEM.md`, `PRODUCT_DECISIONS.md`, `SECURITY_AND_TRUST.md`, `IMPLEMENTATION_PLAN.md`, `PACKAGE_CONTRACTS.md`, and `PLAN_ACCEPTANCE.md` before editing.
...
Use the source-precedence table in `README.md` when documents differ. Record major technical choices as Architecture Decision Records. Before accepting a ticket for implementation, verify its reason, evidence, current plan citations, plan-conformance classification, mapped acceptance criteria, verification, dependencies, and non-goals; never use a ticket to silently change the plan.
