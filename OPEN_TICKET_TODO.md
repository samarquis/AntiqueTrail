...
Before a reviewer can begin, the implementation system must push the candidate branch, create a draft pull request, and complete the final review-request commit described in `OPENCODE_TICKET_REVIEW_TODO.md`. The commit changes this ticket's row to the required `COMPLETE IN PR #<PR>` `[x]` form and changes the matching review-queue row to `[x]`. The review request must name the draft PR URL and the exact final head SHA. `PR: NONE`, a placeholder, pending number, or ready PR is invalid. The implementation and reviewer identities must differ. A new commit after the request invalidates the request until the implementation system posts a new `[x]` handoff for the new SHA.

The reviewer must be a separate system that did not author the implementation. Give it the ticket URL/number, exact base SHA, candidate SHA, draft PR, diff, cited plan headings, `gates/issue-<N>.md`, and test/evidence paths. The reviewer records its claim and verdict as exact-SHA comments on both issue and PR. It must not edit the candidate branch because doing so changes the candidate. Require both review lanes:
...
The reviewer must report findings with file/line or evidence location and severity. The implementation system fixes every valid in-scope finding on the same ticket, reruns affected checks, commits the fixes, updates `$ticketCandidateSha`, and posts a new `[x]` handoff for the new full diff. A genuinely independent or out-of-scope finding becomes a governed successor issue appended to `Ordered tickets` in a separate reviewed bookkeeping pull request; it must not be silently fixed or used to close an unmet acceptance criterion. Never label a review finding or repository-resolvable defect `[!]`; only the dated external blocker protocol can do that. Record reviewer identity, reviewed base/head SHAs, both lanes, every finding/disposition, and reruns at `docs/evidence/issue-<N>/independent-review.md`; commit that history. Continue until the separate reviewer posts an exact-SHA pass receipt. Self-review, a test pass, or a generic “looks good” does not satisfy this gate.
...
Commit and push that TODO update in the same final review-request commit as the matching review-queue `[x]` state. Re-run the plan-governance tests, `git diff --check`, and any check affected by the final diff. Update `$ticketCandidateSha`. The separate reviewer reviews this final base-to-head diff, including the TODO change and independent-review history, then posts a `FINAL INDEPENDENT REVIEW — PASS` or `FINAL INDEPENDENT REVIEW — FAIL` comment on both issue and PR, naming that exact head SHA. Link the exact-head pass receipt in closure evidence. Do not create a self-referential receipt commit and do not change another row.
