...
| Pilot Consent Receipt | None | Pending Partner Identity reads own receipt | None | Read own receipt | None | Read only; no update/delete | Append, version, deliver; no update/delete |
...
- An Administrator creates the first Pilot Store Record after consent using owner-confirmed name, address, phone, website, hours, description, and categories, with provenance and verification date. It is visible only to invited Private Beta participants. Representative-managed versus controlled-field rules remain enforced. Photos, ratings/reviews, events, owner responses, and analytics are excluded. Approved 2026-07-27.
...
- Regional growth sequence is Topeka city limits first, nearby small-community antique/vintage shops second, and a larger metro such as Kansas City only after the small-community stage passes a future review. Communities are added one at a time to build traction; the exact boundary and gate remain unresolved. Approved 2026-07-28.
...
- Pilot consent is captured on a phone-friendly screen with a plain-language summary, full-policy links, separate required acknowledgments, typed name/title/store, verified email, timestamp, invitation ID, and policy version. The Pilot Consent Receipt is immutable, emailed to the owner as a receipt/PDF without internal evidence, and cannot be edited by Administrators. Material term changes require fresh consent. Final wording receives legal review before external use. Approved 2026-07-27.
...
2. Exact Small-Community Expansion boundary, town-selection rule, and success gate; Regional Public MVP is Topeka city limits and larger-metro expansion follows later.
